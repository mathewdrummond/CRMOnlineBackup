...
if is_true "${AI_ENABLED:-true}"; then
  service_args=(postgres qdrant ollama joinerflow-server joinerflow-client joinerflow-clock-client caddy)
  container_names=(joinerflow-postgres joinerflow-qdrant ollama joinerflow-server joinerflow-client joinerflow-clock-client joinerflow-proxy)
fi
