/** @type {import('tailwindcss').Config} */
module.exports = {
  darkMode: ["class"],
  content: ["./index.html", "./src/**/*.{ts,tsx,js,jsx}"],
  safelist: [
    'bg-blue-500', 'bg-blue-100', 'text-blue-700', 'border-blue-200',
    'bg-amber-500', 'bg-amber-100', 'text-amber-700', 'border-amber-200',
    'bg-emerald-500', 'bg-emerald-100', 'text-emerald-700', 'border-emerald-200',
    'bg-red-500', 'bg-red-100', 'text-red-700', 'border-red-200',
    'bg-purple-500', 'bg-purple-100', 'text-purple-700', 'border-purple-200',
    'bg-slate-500', 'bg-slate-100', 'text-slate-700', 'border-slate-200',
    'bg-cyan-500', 'bg-cyan-100', 'text-cyan-700', 'border-cyan-200',
    'bg-pink-500', 'bg-pink-100', 'text-pink-700', 'border-pink-200',
    'bg-orange-500', 'bg-orange-100', 'text-orange-700', 'border-orange-200',
    'bg-teal-500', 'bg-teal-100', 'text-teal-700', 'border-teal-200',
  ],
  theme: {
    extend: {
      fontFamily: {
        sans: ['var(--font-sans)'],
        heading: ['var(--font-heading)'],
      },
      borderRadius: {
        lg: 'var(--radius)',
        md: 'calc(var(--radius) - 2px)',
        sm: 'calc(var(--radius) - 4px)',
      },
      boxShadow: {
        jf: 'var(--jf-shadow-soft)',
        'jf-raised': 'var(--jf-shadow-raised)',
        'jf-overlay': 'var(--jf-shadow-overlay)',
      },
      colors: {
        background: 'hsl(var(--background))',
        foreground: 'hsl(var(--foreground))',
        card: {
          DEFAULT: 'hsl(var(--card))',
          foreground: 'hsl(var(--card-foreground))',
        },
        popover: {
          DEFAULT: 'hsl(var(--popover))',
          foreground: 'hsl(var(--popover-foreground))',
        },
        primary: {
          DEFAULT: 'hsl(var(--primary))',
          foreground: 'hsl(var(--primary-foreground))',
        },
        secondary: {
          DEFAULT: 'hsl(var(--secondary))',
          foreground: 'hsl(var(--secondary-foreground))',
        },
        muted: {
          DEFAULT: 'hsl(var(--muted))',
          foreground: 'hsl(var(--muted-foreground))',
        },
        accent: {
          DEFAULT: 'hsl(var(--accent))',
          foreground: 'hsl(var(--accent-foreground))',
        },
        destructive: {
          DEFAULT: 'hsl(var(--destructive))',
          foreground: 'hsl(var(--destructive-foreground))',
        },
        border: 'hsl(var(--border))',
        input: 'hsl(var(--input))',
        ring: 'hsl(var(--ring))',
        chart: {
          1: 'hsl(var(--chart-1))',
          2: 'hsl(var(--chart-2))',
          3: 'hsl(var(--chart-3))',
          4: 'hsl(var(--chart-4))',
          5: 'hsl(var(--chart-5))',
        },
        sidebar: {
          DEFAULT: 'hsl(var(--sidebar-background))',
          foreground: 'hsl(var(--sidebar-foreground))',
          primary: 'hsl(var(--sidebar-primary))',
          'primary-foreground': 'hsl(var(--sidebar-primary-foreground))',
          accent: 'hsl(var(--sidebar-accent))',
          'accent-foreground': 'hsl(var(--sidebar-accent-foreground))',
          border: 'hsl(var(--sidebar-border))',
          ring: 'hsl(var(--sidebar-ring))',
        },
        millbrook: {
          offwhite: 'hsl(var(--jf-warm-off-white))',
          limestone: 'hsl(var(--jf-limestone))',
          'limestone-soft': 'hsl(var(--jf-limestone-soft))',
          clay: 'hsl(var(--jf-clay))',
          taupe: 'hsl(var(--jf-taupe))',
          walnut: 'hsl(var(--jf-walnut))',
          'walnut-deep': 'hsl(var(--jf-walnut-deep))',
          bronze: 'hsl(var(--jf-bronze))',
          charcoal: 'hsl(var(--jf-charcoal-brown))',
          sage: 'hsl(var(--jf-muted-sage))',
          amber: 'hsl(var(--jf-muted-amber))',
          red: 'hsl(var(--jf-dusty-red))',
          blue: 'hsl(var(--jf-slate-blue))',
        },
      },
      keyframes: {
        'accordion-down': {
          from: { height: '0' },
          to: { height: 'var(--radix-accordion-content-height)' },
        },
        'accordion-up': {
          from: { height: 'var(--radix-accordion-content-height)' },
          to: { height: '0' },
        },
      },
      animation: {
        'accordion-down': 'accordion-down 0.2s ease-out',
        'accordion-up': 'accordion-up 0.2s ease-out',
      },
    },
  },
  plugins: [require("tailwindcss-animate")],
};
